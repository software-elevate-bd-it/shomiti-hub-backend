...
export class SchedulerService {
  ...
  async handleDailySummary() {
    ...
    for (const tenant of tenants) {
      ...
      await this.prisma.statsSummary.upsert({
        where: { somiteeId_date_periodType: { somiteeId: tenant.id, date: startOfDay, periodType: 'daily' } },
        create: {
          somiteeId: tenant.id,
          date: startOfDay,
          periodType: 'daily',
  ...
  async handleMonthlyAggregation() {
    ...
    for (const tenant of tenants) {
      const [payments, expenses] = await Promise.all([
        this.prisma.payment.aggregate({
          where: { somiteeId: tenant.id, paymentDate: { gte: firstDayOfMonth, lt: firstDayNextMonth } },
          _sum: { amount: true }
        }),
        this.prisma.expense.aggregate({
      ...
      await this.prisma.statsSummary.upsert({
        where: { somiteeId_date_periodType: { somiteeId: tenant.id, date: firstDayOfMonth, periodType: 'monthly' } },
        create: {
          somiteeId: tenant.id,
