  ...
  async stats(somiteeId: number) {
    try {
      ...
      if (error instanceof Error) {
        console.error('dashboard.service.service.stats error:', {
          message: error.message,
          stack: error.stack,
          somiteeId: somiteeId,
        });
      } else {
        console.error('dashboard.service.service.stats unknown error:', error);
      }
      ...
      throw new InternalServerErrorException('Failed to stats');
    }
  }

  async memberStats(somiteeId: number, userId: number) {
    try {
      ...
      if (error instanceof Error) {
        console.error('dashboard.service.service.memberStats error:', {
          message: error.message,
          stack: error.stack,
          somiteeId: somiteeId,
          userId: userId,
        ...
        console.error('dashboard.service.service.memberStats unknown error:', error);
      }
      ...
      throw new InternalServerErrorException('Failed to memberStats');
    }
  }
}
